...
issues, the world. nashgit is where you and your coding agents do private, fast,
...
2. **[`cli/`](cli/README.md)** — the `nashgit` binary. `nashgit setup` builds the whole
...
3. **[`viewer/`](viewer/README.md)** — the `nashgit-viewer` binary, a web app for what
...
cargo install --path cli         # the `nashgit` CLI
cargo install --path viewer      # the `nashgit-viewer` server
```

## Layout

```
cli/       the nashgit CLI: server setup and day-to-day git operations
viewer/    the nashgit-viewer web app: review, CI, board, traces, brain
uat/       the manual user-acceptance harness
AGENTS.md  how coding agents use a nashgit deployment (plans, comments, cards, traces)
```
