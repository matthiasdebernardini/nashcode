...
| `viewer/SPEC.md` (Context section), `viewer/src/context.rs` (was `transcripts.rs`), `viewer/src/web/api.rs` (context routes), `viewer/src/brain.rs` (`memory`), `viewer/src/mirror.rs` (`RESERVED_ROUTES`), `viewer/tests/context.rs`, `viewer/tests/brain.rs`, `cli/src/commands/context.rs` (new), `cli/src/commands/brain.rs`, `bin/context-*`, `bin/install-skill`, `.claude/skills/context-digest/`, `extension/mapping.js`, `extension/README.md`, `AGENTS.md` (Context section), `viewer/NOTES.md` | context session (branch `feat/context`) | `plans/context.md`: `POST /:repo/context/:kind` replaces `/transcripts`; brain `memory`; CLI; digest runner; email pusher |
| `viewer/SPEC.md` (People section), `viewer/src/people.rs` (new), `viewer/src/web/api.rs` (`/people` routes), `viewer/src/brain.rs` (`people`), `viewer/src/mirror.rs` (`RESERVED_ROUTES`), `viewer/tests/people.rs`, `cli/src/commands/people.rs` (new), `bin/context-email`, `extension/lib/calendar.js`, `extension/lib/people.js` (new), `extension/mapping-core.js`, `extension/mapping.js`, `people/` (new workspace member), `Cargo.toml` (members), `AGENTS.md` (People section), `viewer/NOTES.md` | people session (branch `feat/context`) | `goals/people/goal.md`: one `~/.nashcode/people.json` routes Meet, Gmail and iMessage to a project; `PUT /people`, `GET /people/route`; `nashcode people`; GPUI editor |
